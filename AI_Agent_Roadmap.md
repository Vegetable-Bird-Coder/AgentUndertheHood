...
| 2026-04-12 | v3.1 | 完成 M3.5「Agent Evals」：实现 Eval 三要素（MockToolRegistry + DeterministicGrader + LLMJudge + EvalHarness）；确立 Mock 的核心价值是"消除外部噪音保证可重复性"而非"方便测试"；发现并定位架构级 Bug：Judge 看不到 call_log 导致 rubric 越界评估分数污染，修法是职责分离（确定性问题归 Grader，语义问题归 Judge）；实验结论：Baseline vs ReAct Δ=+0.037 在噪声范围内无显著差异，验证 ReAct 优势场景是"推理透明度"而非"工具调用准确率"；确立 Eval Ceiling Problem：简单 case 无法区分架构能力边界；理解 LLM-as-Judge 三大可靠性边界：信息不完整/随机噪声/事实盲区 |
